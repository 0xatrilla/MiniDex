...
extension CodexService {
    ...
    // Clears the remembered server URL when the user resets the connection.
    func clearSavedServerConnection() {
        SecureStore.deleteValue(for: CodexSecureKeys.serverURL)
        SecureStore.deleteValue(for: CodexSecureKeys.legacyBridgeURL)
        serverURL = nil
    }
    ...
    func initializeSession() async throws {
        ...
        let clientInfo: JSONValue = .object([
            "name": .string("minidex_ios"),
            "title": .string("MiniDex iOS"),
    ...
    // Classifies socket failures so transient disconnects reconnect without forgetting the saved server URL.
    func handleReceiveError(
        _ error: Error,
        bridgeCloseCode: NWProtocolWebSocket.CloseCode? = nil
    ) {
        if Task.isCancelled {
            return
        }
...
extension CodexService {
    ...
    func userFacingConnectError(error: Error, attemptedURL: String, host: String?) -> String {
        if let nwError = error as? NWError {
            ...
            case .posix(let code) where code == .ECONNREFUSED:
                return "Connection refused by companion server at \(attemptedURL)."
            case .posix(let code) where code == .ETIMEDOUT:
                return "Connection timed out. Check server/network."
            case .dns(let code):
                return "Cannot resolve server host (\(code)). Check the companion address."
            default:
                break
            }
        }
    ...
    func presentConnectionErrorIfNeeded(_ error: Error, fallbackMessage: String? = nil) {
        ...
        // Preserve a more specific pairing message instead of replacing it with a generic disconnect.
