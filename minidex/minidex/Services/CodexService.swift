...
final class CodexService {
    ...
    // Connection persistence for the last successful Codex app-server URL.
    var serverURL: String?

    // --- Internal wiring ------------------------------------------------------

    var webSocketConnection: NWConnection?
    let webSocketQueue = DispatchQueue(label: "MiniDex.WebSocket", qos: .userInitiated)
    ...
    init(
        ...
        // Restore the saved Codex app-server URL from Keychain.
        self.serverURL = SecureStore.readString(for: CodexSecureKeys.serverURL)
            ?? SecureStore.readString(for: CodexSecureKeys.legacyBridgeURL)
    }

    // Remembers whether we can offer reconnect without forcing the user to re-enter a server URL.
    var hasSavedServerConnection: Bool {
        normalizedSavedServerURL != nil
    }

    // Normalizes the persisted server URL before reuse in reconnect flows.
    var normalizedSavedServerURL: String? {
        serverURL?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .nilIfEmpty
    }
